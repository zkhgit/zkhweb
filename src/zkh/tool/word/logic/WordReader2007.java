 ...
 */
public class WordReader2007 {

	private static Logger logger = LoggerFactory.getLogger(WordReader2007.class);
	...
	// 当前表格(的行数据)
	private List<XWPFTableRow> tableRows = null;
	// 当前表格总记录行数
	private int rows = 0;
	// 当前表格当前行的列集合
	private List<XWPFTableCell> row = null;
	// 当前行下标（从0开始）
	...
	public <T extends Object> List<T> readTable(String elementName, FileInputStream fileInputStream, File xmlFile) throws Exception {
	    ...
	    while (it.hasNext()) {
	    	...
	    	this.tableRows = xWPFTable.getRows();
	    	// 获取行总数
	        this.rows = this.tableRows.size();
	        // 得到数据list
	...
	private <T extends Object> List<T> getList(Element element) {
		...
		try {
	        ...
	        for (int i = 1; i < rows; i++) {
	        	// 获取word一行数据
	            this.row = tableRows.get(i).getTableCells();
	            // 创建对象
	...
	private Object getCellValue(Class<?> clazz, Class<?> fieldClass, Element element, int num) {
		try {
			...
			// 得到word行中指定列
			XWPFTableCell cell = this.row.get(cellIndex);
			// 取得单元格的内容
	 ...
	 * @return
	 */
	private boolean iSRowNull(List<XWPFTableCell> row) {
		try {			
			XWPFTableCell cell = null;
			for (int i = 0, len = row.size(); i < len; i++) {
				cell = row.get(i);
				if(cell == null || StringUtils.isBlank(cell.getText())) {
					return true;
				}
			}
		} catch (Exception e) {
