...
/**
 * WebSocket服务器端-案例一
 * 描述：@ServerEndpoint 一个类层次的注解，它的功能主要是将目前的类定义成一个websocket服务器端,注解的值将被用于监听用户连接的终端访问URL地址,客户端可以通过这个URL来连接到WebSocket服务器端。
 * 
 * 赵凯浩
 * 2019年1月8日 上午8:34:43
 */
@ServerEndpoint(value = "/webSocketServer", configurator = WebSocketConfiguration.class)
@Component
public class WebSocketServer {
	...
	public void onOpen(Session webSocketSession, EndpointConfig config) {
		...
		if(userInfo!=null) {			
			try {
				...
				for (WebSocketServer chat : webSocketSet) {
				...
				addOnlineCount();
				// 群发消息，通知所有人
				sendAll(Constants.ONLINE_MSG.replace("LOGINNAME", this.userInfo.getAccount()));
	...
	public void onClose() {
		try {
			...
			subOnlineCount();       
			// 群发消息，通过所有人
			sendAll(Constants.OFFLINE_MSG.replace("LOGINNAME", this.userInfo.getAccount()));
	 ...
	 * @throws Exception
	 */
	public void sendToUsers(String message, String[] userIds) throws Exception{
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("sendTime", getDateTime(null));
		map.put("onlineCount", onlineCount);
		map.put("sessionId", this.sessionId);
		...
		for (WebSocketServer item : webSocketSet) {
			for (int i = 0; i < userIds.length; i++) {				
				if(userIds[i].equals(this.userInfo.getUserId())) {				
					item.sendMessage(ResultData.success(message, map));
				}
			}
		}
