...
public class ExcelReader {
	 ...
	 * @throws Exception
	 */
	public <T extends Object> List<T> readExcel(File xmlFile, String elementName, Workbook workbook) {
		// 最终组装的数据list
		...
		try {
			...
			logger.error("public <T extends Object> List<T> readExcel(String elementName, Workbook workbook)");
	...
	private <T> T setValue(T obj,Class<?> clazz,Row row, Element element, int num) {
		try {
			...
			/**2、赋值**************************************************************************************/
			setMethod.invoke(obj, new Object[]{getCellValue(clazz, fieldMethodReturnClass, row, element, num)});
			/**2、赋值**************************************************************************************/
		} catch (Exception e) {
	 ...
	 * @throws Exception 
	 */
	private Object getCellValue(Class<?> clazz, Class<?> fieldClass, Row row, Element element, int num) {
		try {
			// Excel当前行的当前列下标（从0开始）
